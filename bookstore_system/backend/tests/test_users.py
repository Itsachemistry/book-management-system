...
def test_admin_create_user(client, init_database, app): # 添加 app fixture
    """测试管理员创建用户"""
    # 创建管理员并登录
    ...
    # 验证响应
    assert response.status_code == 201 # 恢复原来的断言
...
def test_admin_list_users(client, init_database, app): # 添加 app fixture
    """测试管理员列出所有用户"""
    # 创建管理员和普通用户
    admin = User(username='admin', employee_id='ADMIN1', role='SUPER_ADMIN')
    admin.set_password('admin123')
    ...
    # 验证响应
    assert response.status_code == 200 # 恢复原来的断言
    data = json.loads(response.data)
    assert len(data) >= 3  # 至少有三个用户

def test_normal_user_cannot_access_admin_routes(client, init_database, app): # 添加 app fixture
    """测试普通用户无法访问管理员路由"""
    # 创建普通用户
    ...
    # 验证响应 - 应该返回403禁止访问
    assert response.status_code == 403 # 恢复原来的断言

def test_update_own_profile(client, init_database, app): # 添加 app fixture
    """测试用户更新自己的资料"""
    # 创建用户
